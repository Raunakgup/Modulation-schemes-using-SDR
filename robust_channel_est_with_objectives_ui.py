#!/usr/bin/env python3
"""
robust_channel_est_with_objectives_ui.py

This file is the original simulation code (kept as-is) with a Streamlit UI wrapper
at the bottom. Two main actions provided in the UI:
 - Run Once: run the single-frame demo with user-specified M, Nbits, sps, fs, snr_db
 - Run Full Simulation: run the example execution block (all five objectives)

Keep in mind long Monte-Carlo sweeps may take time. The core simulation functions
and plotting helpers are unchanged (except for the UI wrapper that calls them).
"""
...
UI_HELP = """
UI for the robust channel estimation script.

- Enter values for M, Nbits, sps, fs, snr_db and click *Run Once* to execute the single-frame demo with those parameters.
- Click *Run Full Simulation* to execute the example execution block that runs the demo and the five objective sweeps.

Note: Full simulation may take long depending on n_trials values used in the objective functions.
"""


def _show_all_matplotlib_figures_to_streamlit():
    """Helper: fetch all open matplotlib figures and display them in Streamlit."""
    if st is None:
        return
    ...
    for n in nums:
        fig = plt.figure(n)
        st.pyplot(fig)
    # close figures to avoid duplication on subsequent runs
    plt.close('all')


def run_ui():
    if st is None:
        raise RuntimeError('Streamlit is not installed. Please install streamlit to use the UI.')

    st.title('Robust Channel Estimation — UI')
    st.markdown(UI_HELP)

    with st.form('inputs'):
        M = st.number_input('M (PSK order)', value=16, min_value=2, step=1)
        Nbits = st.number_input('Nbits (number of data symbols)', value=2000, min_value=1, step=1)
        sps = st.number_input('sps (samples per symbol)', value=8, min_value=1, step=1)
        fs = st.number_input('fs (sampling frequency, Hz)', value=1_000_000.0, format='%f')
        snr_db = st.number_input('snr_db (dB)', value=10.0, format='%f')
        chpilot_len_bits = st.number_input('ch_pilot_len_bits', value=128, min_value=1, step=1)
        sync_len_bits = st.number_input('sync_len_bits', value=26, min_value=1, step=1)
        seed = st.number_input('seed (use -1 for random)', value=1, step=1)
        with_awgn = st.checkbox('with_awgn', value=True)

        run_once = st.form_submit_button('Run Once: single-frame demo')
        run_full = st.form_submit_button('Run Full Simulation (all objectives)')

    if run_once:
        st.write('Running single-frame demo — this will print INFO lines to server console and produce plots below.')
        seed_val = None if int(seed) < 0 else int(seed)
        results = run_single_frame_demo(Nbits=int(Nbits), M=int(M), sps=int(sps), fs=float(fs), snr_db=float(snr_db), ch_pilot_len_bits=int(chpilot_len_bits), with_awgn=bool(with_awgn), seed=seed_val)
        st.write('Result summary:')
        st.json({k: float(v) if isinstance(v, (np.floating, float)) else (int(v) if isinstance(v, (np.integer, int)) else str(type(v))) for k,v in results.items() if not hasattr(v, 'shape')})
        st.write('Displaying generated plots:')
        _show_all_matplotlib_figures_to_streamlit()

    if run_full:
        st.write('Running full simulation — this will run the demo + the five objective sweeps. This can take time.')
        seed_val = None if int(seed) < 0 else int(seed)
        # 1) demo
        results = run_single_frame_demo(Nbits=int(Nbits), M=int(M), sps=int(sps), fs=float(fs), snr_db=float(snr_db), ch_pilot_len_bits=int(chpilot_len_bits), with_awgn=bool(with_awgn), seed=seed_val)
        st.write('Single-frame demo finished. Summary:')
        st.json({k: float(v) if isinstance(v, (np.floating, float)) else (int(v) if isinstance(v, (np.integer, int)) else str(type(v))) for k,v in results.items() if not hasattr(v, 'shape')})
        _show_all_matplotlib_figures_to_streamlit()

        # 2) objective sweeps (small defaults; user can re-run with different n_trials by editing code)
        fs_val = float(fs)
        M_val = int(M)
        Nbits_val = int(Nbits)

        st.write('Objective (i) — SER vs sps')
        sps_list = [1, 2, 4, 8, 16]
        plot_ser_vs_sps(fs=fs_val, M=M_val, Nbits=Nbits_val, sps_list=sps_list, n_trials=20, snr_db=float(snr_db), with_awgn=bool(with_awgn))
        _show_all_matplotlib_figures_to_streamlit()

        st.write('Objective (ii) — SER vs sync length')
        sync_len_list = [8, 13, 26, 52]
        plot_ser_vs_sync_len(fs=fs_val, M=M_val, Nbits=Nbits_val, sync_len_list=sync_len_list, sps=int(sps), n_trials=20, snr_db=float(snr_db), with_awgn=bool(with_awgn))
        _show_all_matplotlib_figures_to_streamlit()

        st.write('Objective (iii) — SER vs modulation order M')
        plot_ser_vs_M(fs=fs_val, M_list=[2, 4, 8, 16, 32], Nbits=max(1000, Nbits_val//2), sps=int(sps), n_trials=20, snr_db=float(snr_db), with_awgn=bool(with_awgn))
        _show_all_matplotlib_figures_to_streamlit()

        st.write('Objective (iv) — SER vs N')
        plot_ser_vs_N(fs=fs_val, M=M_val, sps=int(sps), N_list=[500, 1000, 2000, 4000], n_trials=20, snr_db=float(snr_db), with_awgn=bool(with_awgn))
        _show_all_matplotlib_figures_to_streamlit()

        st.write('Objective (v) — coarse search for best params (small candidate lists)')
        results_search = find_best_params_for_M(M_list=[4, 16], sps_candidates=[2, 4, 8], N_candidates=[1000, 2000], fs_candidates=[1e6, 2e6], n_trials=10, snr_db=float(snr_db), with_awgn=bool(with_awgn))
        st.write('Top combos (per M):')
        st.write(results_search)


# When the file is run directly (no Streamlit) preserve original behaviour
if __name__ == "__main__":
    # If Streamlit is available and the script is run via `streamlit run`, streamlit will handle execution.
    # If not, we preserve original example execution behaviour.
    if st is not None and any(arg.startswith('streamlit') for arg in []) :
        # placeholder - when running under `streamlit run` the run_ui() is triggered by Streamlit
        pass
    else:
        # Original example execution (unchanged):
