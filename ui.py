...
# (UNCHANGED — all your original functions stay the same)

# [ALL YOUR ORIGINAL FUNCTIONS ABOVE THIS LINE REMAIN EXACTLY AS THEY ARE]
...
def run_once_ui():
    """Run single frame demo with user-entered values."""
    try:
        M = int(entry_M.get())
        Nbits = int(entry_Nbits.get())
    ...
    log_box.insert(tk.END, f"▶ Running single frame demo with: M={M}, Nbits={Nbits}, sps={sps}, fs={fs}, snr={snr_db} dB\n")
    log_box.see(tk.END)
    demo_results = run_single_frame_demo(
        Nbits=Nbits, M=M, sps=sps, fs=fs, snr_db=snr_db, with_awgn=True, seed=1
    )
    log_box.insert(tk.END, "✅ Single frame demo completed. Check figures.\n")
    log_box.see(tk.END)
    plt.show()

def run_full_ui():
    """Run full objective sweeps as in original main."""
    try:
        M = int(entry_M.get())
        Nbits = int(entry_Nbits.get())
        sps = int(entry_sps.get())
    ...
    # Objective (iii)
    plot_ser_vs_M(fs=fs, M_list=[2, 4, 8, 16, 32], Nbits=Nbits, sps=sps, n_trials=20, snr_db=snr_db)

    # Objective (iv)
    plot_ser_vs_N(fs=fs, M=M, sps=sps, N_list=[500, 1000, 2000, 4000], n_trials=20, snr_db=snr_db)

    # Objective (v)
    results = find_best_params_for_M(
        M_list=[4, 16],
        sps_candidates=[2, 4, 8],
        N_candidates=[1000, 2000],
        fs_candidates=[1e6, 2e6],
        n_trials=20,
        snr_db=snr_db,
    )
    log_box.insert(tk.END, "✅ Full simulation completed. Best combos:\n")
    for Mkey, combos in results.items():
        log_box.insert(tk.END, f"M={Mkey}\n")
        for ser, sps_v, N_v, fs_v, Rs_v in combos:
...
# -----------------------------------------------------------------------
root = tk.Tk()
root.title("Robust Channel Estimation Simulator")
root.geometry("500x400")

frame_inputs = ttk.LabelFrame(root, text="Simulation Parameters")
frame_inputs.pack(fill="x", padx=10, pady=10)

# Input fields
ttk.Label(frame_inputs, text="M (modulation order):").grid(row=0, column=0, sticky="w", padx=5, pady=5)
entry_M = ttk.Entry(frame_inputs)
entry_M.grid(row=0, column=1, padx=5, pady=5)
entry_M.insert(0, "16")

ttk.Label(frame_inputs, text="Nbits (data bits):").grid(row=1, column=0, sticky="w", padx=5, pady=5)
entry_Nbits = ttk.Entry(frame_inputs)
entry_Nbits.grid(row=1, column=1, padx=5, pady=5)
entry_Nbits.insert(0, "2000")

ttk.Label(frame_inputs, text="sps (samples per symbol):").grid(row=2, column=0, sticky="w", padx=5, pady=5)
entry_sps = ttk.Entry(frame_inputs)
entry_sps.grid(row=2, column=1, padx=5, pady=5)
entry_sps.insert(0, "8")

ttk.Label(frame_inputs, text="fs (sampling freq):").grid(row=3, column=0, sticky="w", padx=5, pady=5)
entry_fs = ttk.Entry(frame_inputs)
entry_fs.grid(row=3, column=1, padx=5, pady=5)
entry_fs.insert(0, "1e6")

ttk.Label(frame_inputs, text="SNR (dB):").grid(row=4, column=0, sticky="w", padx=5, pady=5)
entry_snr = ttk.Entry(frame_inputs)
entry_snr.grid(row=4, column=1, padx=5, pady=5)
entry_snr.insert(0, "10")

# Buttons
frame_buttons = ttk.Frame(root)
frame_buttons.pack(fill="x", padx=10, pady=10)

btn_run_once = ttk.Button(frame_buttons, text="▶ Run Once", command=run_once_ui)
btn_run_once.pack(side="left", padx=5)

btn_run_full = ttk.Button(frame_buttons, text="📊 Run Full Simulation", command=run_full_ui)
btn_run_full.pack(side="left", padx=5)
